...
<head>
    ...
    <title>Sjabloon bewerken — <?= htmlspecialchars($tenantName) ?></title>
...
<div class="min-h-full flex">
    ...
    <aside id="sidebar" class="hidden lg:flex lg:flex-col w-64 bg-white border-r border-slate-200 fixed inset-y-0 left-0 z-30">
        ...
        <nav class="flex-1 px-3 py-4 space-y-1">
            ...
            <a href="<?= BASE ?>/contract/sjablonen" class="flex items-center gap-3 px-3 py-2 rounded-lg text-sm font-medium bg-brand-50 text-brand-700">
    ...
    <div class="lg:hidden fixed top-0 left-0 right-0 z-40 bg-white border-b border-slate-200 px-4 h-16 flex items-center">
        ...
        <span class="ml-3 font-bold text-slate-900 truncate">Sjabloon bewerken</span>
    </div>

    <!-- Main -->
    <main class="flex-1 lg:ml-64 pt-16 lg:pt-0">
        <div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
            <h1 class="text-2xl font-bold text-slate-900 mb-6">Sjabloon bewerken</h1>

            <form method="POST" action="<?= BASE ?>/contract/sjablonen/<?= $template['id'] ?>" class="space-y-6"><?= \Core\Csrf::field() ?>
                <div class="bg-white rounded-xl border border-slate-200 p-6 space-y-5">
                    ...
                    <div>
                        ...
                        <textarea name="content_html" id="content_html" rows="16" required
                                  class="w-full border border-slate-300 rounded-lg px-3 py-2 text-sm font-mono focus:ring-2 focus:ring-brand-500 focus:border-brand-500"><?= htmlspecialchars($template['content_html']) ?></textarea>
                    </div>
                ...
                <div class="flex items-center gap-3">
                    ...
                    <a href="<?= BASE ?>/contract/sjablonen" class="px-5 py-2.5 bg-white border border-slate-200 text-slate-700 rounded-lg text-sm font-medium hover:bg-slate-50 transition-colors">
                        Annuleren
                    </a>
                </div>
            </form>
