...
<head>
    ...
    <title>Contract Dashboard — <?= htmlspecialchars($tenantName) ?></title>
...
<div class="min-h-full flex">
    ...
    <aside id="sidebar" class="hidden lg:flex lg:flex-col w-64 bg-white border-r border-slate-200 fixed inset-y-0 left-0 z-30">
        ...
        <nav class="flex-1 px-3 py-4 space-y-1">
            <a href="/contract" class="flex items-center gap-3 px-3 py-2 rounded-lg text-sm font-medium bg-brand-50 text-brand-700">
        ...
        <div class="px-3 py-4 border-t border-slate-200">
            ...
            <div class="flex items-center justify-between mt-3 px-3">
                ...
                <a href="/logout" class="text-xs text-slate-400 hover:text-slate-600">Uitloggen</a>
            </div>
        </div>
    </aside>
    ...
    <main class="flex-1 lg:ml-64 pt-16 lg:pt-0">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
            <h1 class="text-2xl font-bold text-slate-900 mb-6">Contract Dashboard</h1>

            <!-- Stats -->
            <div class="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
                <div class="bg-white rounded-xl border border-slate-200 p-5">
                    <p class="text-sm text-slate-500 mb-1">Actieve contracts</p>
                    <p class="text-3xl font-bold text-brand-600"><?= $totalActive ?></p>
                </div>
                <div class="bg-white rounded-xl border border-slate-200 p-5">
                    <p class="text-sm text-slate-500 mb-1">Verlopen</p>
                    <p class="text-3xl font-bold text-red-600"><?= $expired ?></p>
                </div>
                <div class="bg-white rounded-xl border border-slate-200 p-5">
                    <p class="text-sm text-slate-500 mb-1">Tekenen in afwachting</p>
                    <p class="text-3xl font-bold text-amber-500"><?= $pendingSign ?></p>
                </div>
                <div class="bg-white rounded-xl border border-slate-200 p-5">
                    <p class="text-sm text-slate-500 mb-1">Sjablonen</p>
                    <p class="text-3xl font-bold text-slate-700"><?= $totalTemplates ?></p>
                </div>
            </div>
            ...
            <div class="bg-white rounded-xl border border-slate-200">
                <div class="px-6 py-4 border-b border-slate-100">
                    <h2 class="text-lg font-semibold text-slate-900">Verloopt binnenkort</h2>
                    <p class="text-sm text-slate-500">Contracten die binnen 30 dagen verlopen</p>
                </div>
                        ...
                        <table class="w-full text-sm">
                            <thead class="bg-slate-50 text-left">
                                <tr>
                                    ...
                                    <th class="px-6 py-3 font-medium text-slate-500">Vervaldatum</th>
                                    <th class="px-6 py-3 font-medium text-slate-500"></th>
                                </tr>
                            </thead>
                            <tbody class="divide-y divide-slate-100">
                                <?php foreach ($expiringSoon as $c): ?>
                                    <tr class="hover:bg-slate-50">
                                        ...
                                        <td class="px-6 py-4">
                                            <span class="inline-flex items-center gap-1 px-2 py-1 rounded-full text-xs font-medium bg-amber-50 text-amber-700">
                                                <?= date('d-m-Y', strtotime($c['end_date'])) ?>
                                            </span>
                                        </td>
                                        <td class="px-6 py-4 text-right">
                                            <a href="/contract/contracts/<?= $c['id'] ?>" class="text-brand-600 hover:text-brand-700 font-medium">Bekijk</a>
                                        </td>
                                    </tr>
                                <?php endforeach; ?>
                            </tbody>
