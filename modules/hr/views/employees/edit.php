...
<head>
    ...
    <title>Bewerk medewerker — HR Dashboard</title>
...
<div class="min-h-full flex">
    ...
    <div class="flex-1 lg:ml-64">
        <header class="bg-white border-b border-slate-200 h-16 flex items-center px-4 sm:px-6 lg:px-8">
            ...
            <a href="<?= BASE ?>/hr/medewerkers/<?= $employee['id'] ?>" class="text-slate-400 hover:text-slate-600 mr-3">
                <svg class="w-5 h-5" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path d="M15 19l-7-7 7-7"/></svg>
            </a>
            <h1 class="text-lg font-semibold text-slate-900">Bewerk: <?= htmlspecialchars($employee['name']) ?></h1>
        </header>

        <main class="p-4 sm:p-6 lg:p-8 max-w-2xl">
            <form method="POST" action="<?= BASE ?>/hr/medewerkers/<?= $employee['id'] ?>" class="bg-white rounded-xl border border-slate-200 p-6 space-y-5"><?= \Core\Csrf::field() ?>
                <div>
                    <label class="block text-sm font-medium text-slate-700 mb-1">Naam *</label>
                    <input type="text" name="name" value="<?= htmlspecialchars($employee['name']) ?>" required class="w-full px-4 py-2 border border-slate-300 rounded-lg text-sm focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500">
                </div>
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                    <div>
                        <label class="block text-sm font-medium text-slate-700 mb-1">Email</label>
                        <input type="email" name="email" value="<?= htmlspecialchars($employee['email'] ?? '') ?>" class="w-full px-4 py-2 border border-slate-300 rounded-lg text-sm focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500">
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-slate-700 mb-1">Telefoon</label>
                        <input type="text" name="phone" value="<?= htmlspecialchars($employee['phone'] ?? '') ?>" class="w-full px-4 py-2 border border-slate-300 rounded-lg text-sm focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500">
                    </div>
                </div>
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                    <div>
                        ...
                        <select name="department_id" class="w-full px-4 py-2 border border-slate-300 rounded-lg text-sm focus:ring-2 focus:ring-emerald-500">
                            ...
                            <?php foreach ($departments as $d): ?>
                                <option value="<?= $d['id'] ?>" <?= ($employee['department_id'] ?? 0) == $d['id'] ? 'selected' : '' ?>><?= htmlspecialchars($d['name']) ?></option>
                            <?php endforeach; ?>
                        </select>
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-slate-700 mb-1">Functie</label>
                        <input type="text" name="position" value="<?= htmlspecialchars($employee['position'] ?? '') ?>" class="w-full px-4 py-2 border border-slate-300 rounded-lg text-sm focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500">
                    </div>
                </div>
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                    <div>
                        <label class="block text-sm font-medium text-slate-700 mb-1">Salaris</label>
                        <input type="number" name="salary" step="0.01" value="<?= $employee['salary'] ?? '' ?>" class="w-full px-4 py-2 border border-slate-300 rounded-lg text-sm focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500">
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-slate-700 mb-1">Verlofsaldo (dagen)</label>
                        <input type="number" name="leave_balance_days" value="<?= $employee['leave_balance_days'] ?? 25 ?>" class="w-full px-4 py-2 border border-slate-300 rounded-lg text-sm focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500">
                    </div>
                </div>
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                    <div>
                        <label class="block text-sm font-medium text-slate-700 mb-1">Startdatum</label>
                        <input type="date" name="start_date" value="<?= $employee['start_date'] ?? '' ?>" class="w-full px-4 py-2 border border-slate-300 rounded-lg text-sm focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500">
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-slate-700 mb-1">Contract einde</label>
                        <input type="date" name="contract_end" value="<?= $employee['contract_end'] ?? '' ?>" class="w-full px-4 py-2 border border-slate-300 rounded-lg text-sm focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500">
                    </div>
                </div>
                <div>
                    <label class="block text-sm font-medium text-slate-700 mb-1">Status</label>
                    <select name="status" class="w-full px-4 py-2 border border-slate-300 rounded-lg text-sm focus:ring-2 focus:ring-emerald-500">
                        <option value="actief" <?= $employee['status'] === 'actief' ? 'selected' : '' ?>>Actief</option>
                        <option value="inactief" <?= $employee['status'] === 'inactief' ? 'selected' : '' ?>>Inactief</option>
                        <option value="opgezegd" <?= $employee['status'] === 'opgezegd' ? 'selected' : '' ?>>Opgezegd</option>
                    </select>
                </div>
                <div class="flex gap-3 pt-2">
                    ...
                    <a href="<?= BASE ?>/hr/medewerkers/<?= $employee['id'] ?>" class="px-5 py-2 bg-slate-100 text-slate-700 text-sm font-medium rounded-lg hover:bg-slate-200">Annuleren</a>
                </div>
            </form>
        </main>
    </div>
