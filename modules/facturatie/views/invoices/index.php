...
<head>
    ...
    <title>Facturen — Facturatie — HanzeOnline</title>
    ...
    <div class="flex min-h-screen">
        ...
        <div class="flex-1 lg:ml-64">
            <header class="bg-white border-b border-gray-200 px-4 py-3 flex items-center justify-between lg:px-8">
                ...
                <h1 class="text-lg font-semibold text-gray-900">Facturen</h1>
                <a href="<?= BASE ?>/facturatie/facturen/nieuw" class="inline-flex items-center gap-2 bg-blue-500 hover:bg-blue-600 text-white text-sm font-medium px-4 py-2 rounded-lg">
            ...
            <main class="p-4 lg:p-8">
                ...
                <div class="flex flex-wrap gap-2 mb-6">
                    ...
                    ?>
                    <a href="<?= BASE ?>/facturatie/facturen<?= $val ? "?status=$val" : '' ?>" class="px-4 py-2 rounded-lg text-sm font-medium <?= $currentStatus === $val ? 'bg-blue-500 text-white' : 'bg-white border border-gray-200 text-gray-600 hover:bg-gray-50' ?>"><?= $label ?></a>
                    <?php endforeach; ?>
                </div>

                <!-- Search -->
                <form method="GET" action="<?= BASE ?>/facturatie/facturen" class="mb-6">
                    <input type="hidden" name="status" value="<?= htmlspecialchars($currentStatus) ?>">
                    <div class="relative max-w-md">
                        <svg class="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"/></svg>
                        <input type="text" name="search" value="<?= htmlspecialchars($search) ?>" placeholder="Zoek op nummer of klant..." class="w-full pl-10 pr-4 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none">
                    </div>
                </form>
                        ...
                        <table class="w-full text-sm">
                            <thead class="bg-gray-50 text-left">
                                <tr>
                                    ...
                                    <th class="px-6 py-3 font-medium text-gray-500 hidden md:table-cell">Vervaldatum</th>
                                    <th class="px-6 py-3"></th>
                                </tr>
                            </thead>
                            <tbody class="divide-y divide-gray-100">
                                <?php if (empty($invoices)): ?>
                                <tr><td colspan="6" class="px-6 py-8 text-center text-gray-400">Geen facturen gevonden</td></tr>
                                <?php else: ?>
                                ...
                                <tr class="hover:bg-gray-50">
                                    ...
                                    <td class="px-6 py-4 text-gray-500 hidden md:table-cell"><?= $inv['due_date'] ? date('d-m-Y', strtotime($inv['due_date'])) : '—' ?></td>
                                    <td class="px-6 py-4 text-right">
                                        <a href="<?= BASE ?>/facturatie/facturen/<?= $inv['id'] ?>" class="text-gray-400 hover:text-blue-500">
